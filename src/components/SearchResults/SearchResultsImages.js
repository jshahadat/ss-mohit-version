...
function SearchResultsImages({ query }) {
    ...
    useEffect(() => {
        fetch(`https://chatapi.ssebowa.org/full_image/?keyword=${query}`, {
            method: "post",
        })
                    ...
                    <div className="result-slider" style={{ marginY: "10px" }}>
                        <div style={{ margin: "0 -15px" }}>
                            <Swiper
                                ...
                                className="mySwiper"
                            >
                                {Object.keys(imagesData).map((item, index) => (
                                    <SwiperSlide key={index}>
                                        <ImageListItem classes="result-image-wrapper">
                                            <a href={imagesData[item].url} className="result-image-link">
                                                <p className="image-text">
                                                    {imagesData[index]?.title ? `${imagesData[index].title.substring(0, 40)}...` : "Some title"}
                                                </p>
                                                <img src={imagesData[item].img2} className="result-image-item" loading="lazy"
                                                     alt="img" />
                                            </a>
                                        </ImageListItem>
                                    </SwiperSlide>
